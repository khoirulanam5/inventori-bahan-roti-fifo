                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><?= $data['no_hp'] ?></td>
                                        <td>
                                            <a href="#" id="EditSupplier" data-id="<?= $data['id_supplier']; ?>"><span class="badge badge-success"><i class="far fa-edit"></i></span></a>
                                            <a href="#"><span onclick="hapus(this)" data-id="<?= $data['id_supplier'] ?>" class="badge badge-danger"><i class="far fa-trash-alt"></i></span></a>
                                        </td>
                                    </tr>
                                <?php
                                endforeach;
...
if (isset($_GET['success'])) {
    alert_process($_GET['success'], true, "Supplier", $_GET['menu']);
} else if (isset($_GET['fail'])) {
    alert_process($_GET['fail'], false, "Supplier", $_GET['menu']);
}
?>
<!-- Modal -->
<div class="modal fade" id="tambah_data" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="proccess/supplier_tambah.php" method="POST">
...
<script>
    ...
    $(document).on('click', '#EditSupplier', function(e) {
        ...
        $('#modalku').modal('show');
        var fileedit = 'menu/admin/supplier_edit.php';
        $.ajax({
            type: 'POST',
            url: fileedit,
            ...
            .then((result) => {
                if (result.value) {
                    $.ajax({
                        ...
                        url: 'proccess/supplier_hapus.php', // File yang akan memproses data
                        data: 'id=' + id,
                        dataType: "html",
                        success: function(response) {
                            ...
                            window.setTimeout(function() {
                                window.location.replace('?menu=supplier');
                            }, 1000);
                        }
                    });
                }
