                    ...
                    <div class="table-responsive mb-4 mt-4">
                        <table id="zero-config" class="table table-hover" style="width:100%">
                            ...
                            <tbody>
                                <?php
                                $i = 1;
                                $view = list_pengajuan_petugas_gudang();
                                foreach ($view as $data) :
                                    if ($data['status'] == '0') {
                                        $status = '<font style="color:red">Diproses</font>';
                                    } else {
                                        $status = '<font style="color:blue">Diterima</font>';
                                    }
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><button id="LihatSupplier" value="<?= $data['id_pengajuan']; ?>" class="lik">#<?= $data['id_pengajuan']; ?> </button></td>
                                        <td><?= ubah_format_tgl($data['tgl_pengajuan']) ?></td>
                                        <td><?= $status ?></td>
                                        <td>
                                            <?php
                                            if ($data['brg_keluar'] == '1') {
                                                echo '<font style="color:blue">Sudah Dikirim</font>';
                                            } else {
                                            ?>
                                                <a href="#" id="EditSupplier" data-id="<?= $data['id_pengajuan']; ?>"><span class="badge badge-primary">Kirim Bahan Baku</span></a>
...
if (isset($_GET['success'])) {
    echo "<script>
            Swal.fire({
                ...
                text: 'bahan baku diambil dari tanggal yang terlama (hampir exp)',
                confirmButtonText: 'OK'
            }).then(function() {
                window.location = '?menu=" . $_GET['menu'] . "';
            });
          </script>";
    ...
    echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
                text: 'Pengiriman gagal dilakukan!',
                confirmButtonText: 'OK'
            }).then(function() {
                window.location = '?menu=" . $_GET['menu'] . "';
            });
